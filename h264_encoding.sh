...
# Ref at https://samuel.dalesjo.net/2018/01/06/gop-configuration-with-ffmpeg/
ffmpeg -y -f rawvideo -vcodec rawvideo -pix_fmt gray -s 2560x2560 -r 1 -i ${video_input_path} -codec:v libx264 -x264-params keyint=4:min-keyint=4:scenecut=0:bframes=0:b-adapt=0:ref=2:b-adapt=0:open-gop=1 -c:a copy ${video_output_path}
# Check the PSNR
ffmpeg -i ${video_output_path} -s 2560x2560 -pix_fmt gray -r 1 -i ${video_input_path} -s 2560x2560 -pix_fmt gray -r 1 -filter_complex "psnr" -f null /dev/null